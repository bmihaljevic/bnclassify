...
nparams <- function(x) {  
  if(inherits(x, "bnc_multinet")){
    p<-0
    for(i in x$.models){
      p<-p + sum(vapply(params(i), count_cpt_free_params, FUN.VALUE = numeric(1)))
    }
    p/length(x$.models)
  }
  else{sum(vapply(params(x), count_cpt_free_params, FUN.VALUE = numeric(1)))}
}
#' @export
