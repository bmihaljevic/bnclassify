...
To avoid scoring equivalent structures, at each step we selected the $X_i$ to $X_j$ such that $X_i$ is alphabetically (that is, the name of the column in the data set) before $X_j$. A preferable implementation would be to select the arc randomly. 
