---
title: "Introduction to the `bnclassify` package"
...
# Introduction
The `bnclassify` package implements state-of-the art algorithms for learning discrete Bayesian network classifiers from data, as well as functions for using these classifiers for prediction, assessing their predictive performance, and inspecting and analyzing their properties. 

This vignette introduces the available functionalities. Other resources provide additional information:

- `?bnclassify` lists main functionalities.
- `vignette("technical", package="bnclassify")` provides details on the implemented methods.
- `vignette("runtimes", package="bnclassify")` discusses some issues affecting algorithms' runtimes.
...
If the network is not displaying properly, e.g., with node names overlapping in large networks, you may try different layout types and font sizes. 
```{r, fig.height=4}
...
For more functions to query a network structure, see `?bnc_dag_object`.
...
For more functions for querying a `bnc_bn` object, see `?bnc_bn_object`
...
The following uses `gRain` to ge the marginal probability of the `buying` feature:
```{r}
...
Finally, to cross-validatate multiple classifiers at once pass a list of `bnc_bn` objects to `cv`. 
