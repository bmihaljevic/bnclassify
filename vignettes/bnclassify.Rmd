...
## Intro
This is mainly for complete data but works for incomplete, too. Only discrete.
...
You can also have the convenience `bnc` function bnc('class', car, tan) is equivalent to tan() lp(x, ..)
...
r
set.seed(0)
microbenchmark(r <- bnc_cv(list(a), car, dag = FALSE, k = 10))
r
```
...
## Incomplete data
You can use this package with incomplete data, but it is very slow. Example with the voting data set...

Best not to use incomplete data with wrapper classifiers.
