...
whereas the CPT for `buying` is 
```{r}
params(b)$class
```

Also, we can compute several network scores, such as its log-likelihood: 
```{r}
logLik(b, car)
```
...
## Parameter weighting
For naive Bayes, one can combine maximum likelihood and Bayesian parameter estimation with posterior feature parameter weighting. This involves exponentiating the features' CPT entries by a value between 0 and 1 and can alleviate some of the negative effects of redundancy. See `?lp` for more information.


```{r}
a <- nb('class', car)
b <- lp(a, car, smooth = 1)
c <- lp(a, car, smooth = 1, awnb_trees = 20, awnb_bootstrap = 0.5)
...
ta <- lp(t, car, smooth = 1, awnb_trees = 10, awnb_bootstrap = 0.5)
params(t)$buying
params(ta)$buying
```

## Exact model averaging for naive Bayes

We can use the model averaged naive Bayes method. 
```{r}
d <- lp(a, car, smooth = 1, manb_prior = 0.3)
arc_posterior(d)
```
...
b <- lp(dag, voting, smooth = 1, awnb_trees = 40, awnb_bootstrap = 0.5)
...
Assuming you have mlr installed, call `as_mlr()` to convert a `bnc_bn` to an mlr *learner*. This allows you to use mlr functionalities: selecting features, benchmarking, etc. Below, we use a naive Bayes to perform forward wrapper feature selection. 

Fist, we learn a naive Bayes and wrap it as an `mlr learner'.
...
Then we use the regular `mlr` code to set up and perform forward
...
The above has selected just the buying price feature.
```{r}
sfeats$x
```
