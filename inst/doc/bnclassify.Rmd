...
Evaluating multiple classifiers can by only a little more costly than evaluating the largest among them, because `cv` takes advantage of local distributions shared among the classifiers, learning them from data only once.

## Multiple cross-validation
`bnclassify` cross-validation takes advantage of the shared local distributions among the compared classifiers. Namely, two networks over the some domain may have these distributions are only fitted from data and accessed onces when evaluating multiple structures.
